...
tarifas_historico <- importar_totales_tarifas() %>% 
  mutate(nacionales = str_detect(header_1, "Nacionales")) %>% 
  mutate(tipo = if_else(nacionales,
                        str_match(header_1, PAT_NACIONALES)[,3],
                        str_match(header_1, PAT_INTERNACIONALES)[,6])) %>% 
  mutate(destino = if_else(nacionales, 
                           NA_character_, 
                           str_match(header_1, PAT_INTERNACIONALES)[,5]))
...
tarifas_historico %>% 
  ...
  filter(header_1 == "Nacionales Comunes") %>% 
