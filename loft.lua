...
local function addFigRef(div)
  ...
  if not options.lof2 then return nil end
...
local function appendLoft(doc)
  ...
  local lofHeader
  local lofClasses = {"lof", "unnumbered"}
  local idprefix = options.idprefix or ""

  if options.lof2 then
        lofHeader =
      pandoc.Header(1,
                    {table.unpack(options["lof-title"])},
                    pandoc.Attr(idprefix .. "LOF", lofClasses, {})
      )
    table.insert(doc.blocks, lofHeader)
    table.insert(doc.blocks, pandoc.BulletList(figuresList))
...
return {
  {Meta = getMeta},
  {Div = addFigRef, Image = addFigRef2,  Pandoc = appendLoft}
}
